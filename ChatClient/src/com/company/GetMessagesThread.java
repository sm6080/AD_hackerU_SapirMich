...
// מביא כל פעם את ההודעה שלי מהשרת , גם פה משרשר את הסיסמא והשם
    @Override
    public void run() {
        while (go) {
            ...
            try {
                ...
                outputStream.write(GET_MESSAGES);//write to the server
                ...
                while ((messageLength = inputStream.read()) != -1) {
                    ...
                    int senderLenth=inputStream.read();
                    if (senderLenth==-1)
                        continue;
                    byte[] senderBytes=new byte[senderLenth];
                    actuallyRead=inputStream.read(senderBytes);
                    if (actuallyRead!=senderLenth)
                        continue;
