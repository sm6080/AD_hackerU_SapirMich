...
public class ClientThread extends Thread {
    ...
    private void sendMessage() throws IOException {
        ...
        int messageLength = inputStream.read(); // קורא מחרוזת את האורך
        if (messageLength == -1)
            return;
    ...
    private boolean validUser(User user) {
        ...
        try (Connection connection = DB.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(
                    "SELECT USER_NAME, PASSWORD FROM USERS_CHAT WHERE USER_NAME=? AND "
            );
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if (resultSet.next()) {
                    if (user.getPassword().equals(resultSet.getString(2)))
                        return true;
                    else
                        return false;
                }
    ...
    private void getMessages() throws IOException {  //כמה הודעות נקראו
        User user = readUserFromStream();
        if (!validUser(user))
            return;
    ...
    private void insertUsers(User user) {
        try (Connection connection = DB.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO USERS_CHAT (USER_NAME, PASSWORD) VALUES(?,?)"
            );
            statement.setString(1, user.getUserName());
            statement.setString(2, user.getPassword());
            statement.executeQuery();
